...
pub fn bytes_to_gb_i32(bytes: u64) -> i32 {
    // Convert to GB with 3 decimal places of precision
    // Multiply by 1000 to preserve 3 decimal places
    ((bytes as f64 * 1000.0) / 1_073_741_824.0).round() as i32
}

// At server, we decode the memory usage from i32 to f32 to get correct memory usage
#[allow(dead_code)]
pub fn gb_i32_to_f32(gb: i32) -> f32 {
...
pub fn get_memory_info() -> (i32, i32) {
    ...
    (
        bytes_to_gb_i32(program_memory),
        bytes_to_gb_i32(total_memory),
    )
}
