...
pub fn get_or_generate_prover_id(node_id: &str) -> String {
    //if node_id is empty, generate a default id
    let prover_id = if node_id.is_empty() {
        generate_default_id()
    } else {
        node_id.to_string()
    };
    ...
    if !nexus_dir.exists() {
        return handle_first_time_setup(&nexus_dir, &prover_id_path, &prover_id);
    }

    //print how to find the prover-id file
    ...
    match read_existing_prover_id(&prover_id_path) {
        ...
        Ok(id) => {
            println!(
                "Successfully read existing prover-id '{}' from file: {}",
                id,
                prover_id_path.to_string_lossy()
            );
            id
        }
        ...
        Err(e) => {
            ...
            handle_read_error(e, &prover_id_path, &prover_id);
            prover_id
        }
    }
}
